...
require 'app/vendor/phpmailer/phpmailer/src/PHPMailer.php'; // Reemplaza 'path/to' con la ubicación real de PHPMailer.php
require 'app/vendor/phpmailer/phpmailer/src/SMTP.php'; // Reemplaza 'path/to' con la ubicación real de SMTP.php
require 'app/vendor/phpmailer/phpmailer/src/Exception.php'; // Reemplaza 'path/to' con la ubicación real de Exception.php
//////////////////////////////////////////////////
...
function get_siteemail() {
  return 'todohidraulicos@flowcontrol.sotdatalabs.com';
}
...
d M, Y time
**/
function format_date($date_string, $type = 'd M, Y time')
{
...
function send_email($from, $to, $subject, $body, $alt = null, $bcc = null, $reply_to = null, $attachments = []) {
	try {
		...
		if (defined('PHPMAILER_SMTP') && PHPMAILER_SMTP === true) {
			...
			$mail->Username   = 'todohidraulicos@flowcontrol.sotdatalabs.com';                               //SMTP username
			$mail->Password   = '20*TodoHidr@ulico24#';                               //SMTP password
...
function get_user($key = null) {
    ...
    // Comprobar si la clave es 'horario' para obtener la imagen del usuario
    if ($key === 'horario') {
        // Comprobar si el campo 'horario' existe en los datos del usuario y devolverlo
        return isset($user['horario']) ? $user['horario'] : null;
    }
...
function get_user_profile($key = null) {
    if (!isset($_SESSION['user_session'])) return false;
    ...
    if ($key === 'horario') {
        // Verificar si la imagen del horario está almacenada en el usuario
        if (isset($user['horario']) && !empty($user['horario'])) {
            // Aquí asumimos que la imagen del horario es una URL
            $horarioImage = $user['horario'];
            return $horarioImage;
        } else {
            // La imagen del horario no está disponible en el usuario
            return false;
        }
    }
