...
class ajaxController extends Controller {
  ...
  {
    try {
      if (!check_posted_data(['csrf', 'id_usuario'], $_POST) || !Csrf::validate($_POST["csrf"])) {
        throw new Exception(get_notificaciones());
      }

      $id_usuario = clean($_POST["id_usuario"]);

      if (!$usuario = usersModel::by_id($id_usuario)) {
        throw new Exception('No existe el usuario en la base de datos.');
      }

      if ($usuario['status'] === 'suspendido') {
        throw new Exception(sprintf('<b>%s</b> ya se encuentra suspendido.', $usuario['nombre_completo']));
      }

      if ($usuario['status'] === 'pendiente') {
        throw new Exception(sprintf('No se puede suspender al usuario <b>%s</b>.', $usuario['nombre_completo']));
      }

      // Suspension del usuario
      if (usersModel::suspender($id_usuario) === false){
        throw new Exception(get_notificaciones(4));
      }

      // Email suspensión
      mail_suspension_cuenta($id_usuario);

      $msg = sprintf('El alumno <b>%s</b> ha sido suspendido  con éxito.', $usuario['nombre_completo']);

      json_output(json_build(200, $usuario, $msg));
  ...
  function remover_suspension_usuario()
  {
    try {
      if (!check_posted_data(['csrf', 'id_usuario'], $_POST) || !Csrf::validate($_POST["csrf"])) {
        throw new Exception(get_notificaciones());
      ...
      if (!$usuarios = usersModel::by_id($id_usuario)) {
        throw new Exception('No existe el usuario registrado en la base de datos.');
      }

      if ($usuarios['status'] !== 'suspendido') {
        throw new Exception(sprintf('<b>%s</b> no se encuentra suspendido.', $usuarios['nombre_completo']));
      }

      if ($usuarios['status'] === 'pendiente') {
        throw new Exception(sprintf('No se puede remover la suspensión del alumno <b>%s</b>.', $usuarios['nombre_completo']));
      }

      // Retiro de suspension del alumno
      if (usersModel::remover_suspension($id_usuario) === false){
        throw new Exception(get_notificaciones(4));
      }

      // Email de remover suspensión
      mail_retirar_suspension_cuenta($id_usuario);

      $msg = sprintf('Se ha retirado la suspensión del usuario <b>%s</b> con éxito.', $usuarios['nombre_completo']);
      $usuario = usersModel::by_id($id_usuario);
